    ...
    <div className="relative">
      <Slider ref={sliderRef} {...settings}>
        {slides.map((slide, index) => (
          <div
            key={index}
            className="flex items-center justify-center w-full border-solid border-2"
          >
            <div className="flex flex-row w-full">
              ...
              <div className="flex flex-col md:w-2/3 h-full p-4">
                {slide.headings ? (
                  <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4">
                    {slide.descriptions.map((desc, i) => (
                      <div key={i} className="flex flex-col flex-1">
                        <div className="font-bold text-3xl p-2 text-center text-secondary">
                          {desc.heading}
                        </div>
                        <div className="text-center text-btnHover p-5 text-xl">
                          {desc.text}
                        </div>
                      </div>
                    ))}
                  ...
                  <>
                    <div className="flex-auto font-bold text-3xl p-2 text-center text-secondary">
                      {slide.heading}
                    </div>
                    <div className="flex-auto text-center text-btnHover p-5 text-xl">
                      {slide.description}
                    </div>
                  </>
                )}
